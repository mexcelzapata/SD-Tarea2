...
def consumer_from_beginning(topic):
    ...
    partitions = [TopicPartition(topic,p) for p in consumer.partitions_for_topic(topic)] #linea salvadora
...
def enviarEmails():
    ...
    for val in arr_resumenDiario_json:
        msg = "\nBuenas tardes, en "+str(val['cantidadDeOrdenes'])+" ordenes, la cantidad de sopaipillas vendidas el dia de hoy son:" + str(val['cantidadVendida']) + " lo cual corresponde a un total de $" + str(val['total'])
        print(msg)
